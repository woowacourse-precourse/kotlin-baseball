...
fun main() {
    var userChoice = GAME_START
    println("숫자 야구 게임을 시작합니다.")

    while (userChoice == GAME_START) {
        userChoice = baseballGame()
    }
    if (userChoice != GAME_END) throw IllegalArgumentException()
    println("게임 종료")
}
...
fun getBallAndStrike(
    ...
    var ballCount: Int = getBall(computerNum, userNum)
    val strikeCount: Int = getStrike(computerNum, userNum)
...
fun getBall(
    ...
    for (idx in userNum.indices) { // 입력한 숫자 사이즈인 3번 반복한다.
        if (computerNum.contains(userNum[idx])) { // 해당 숫자가 위치 상관없이 포함한다면 볼카운트 1증가
            ballCount += 1
        }
    }
    return ballCount
...
fun printBallAndStrike(ballAndStrike: List<Int>): Boolean {
    val ballCount = ballAndStrike[0]
    val strikeCount = ballAndStrike[1]
