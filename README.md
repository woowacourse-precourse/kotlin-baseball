...
1. 1에서 9까지 서로 다른 임의의 수 3개를 선택받는다. 
   
2. 사용자에게 3자리의 수를 입력받는다.
   이 때, 사용자가 잘못된 값을 입력한 경우 예외처리를 한다.
  - 입력한 값이 3자리가 아닐 때,
  - 입력한 각 자리 수가 1~9가 아닐 때,


3. 정상 범위의 값이 입력되면 입력값이 맞춘 정보를 제시한다.
  - 같은 수 , 같은 자리면 스트라이크
  - 같은 수 , 다른 자리면 볼
  - 같은 수가 없으면 낫싱

4. 숫자를 모두 맞히고 1을 입력하면 게임을 재시작하고 , 2를 입력하면 게임을 종료한다.
