...
1. 내가 입력할 3개의 숫자를 지정, 배열로 저장
2. 컴퓨터의 랜덤값 추출, 배열로 저장
3. 배열끼리 서로 비교하여 스트라이크, 볼, 낫싱을 출력
4. 게임을 맞출때까지 계속 할 수 있도록 반복(for문으로 대체)
5. 3스트라이크가 나왔을 시, 문장 출력과 게임 종료
6. 게임이 종료된 후 재시작과 종료(1: 재시작, 2: 종료)