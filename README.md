...
- [x] 1~9 사이의 숫자 중 서로 다른 임의의 수 3개를 컴퓨터 리스트에 저장
- [x] 사용자에게서 입력 받은 문자열을 정수 리스트로 변환
- [x] NumberBaseballGame 클래스 생성 후, 사용자 입력 숫자와 컴퓨터가 선택한 숫자 비교하여 점수 계산
  - 같은 수가 같은 자리에 있으면 스트라이크, 같은 수가 다른 자리에 있으면 볼 변수 1씩 증가
- [x] 게임 결과 문구 출력
- [x] 게임 과정을 반복하기 위한 while문 추가 후, 점수 리셋 함수 추가
- [x] 재시작(1) 일 때 게임 다시 시작, 종료(2) 일 때 게임 종료하는 함수 추가
- [x] 예외 처리

### ☑️ 예외 처리
- 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException` 발생 후 애플리케이션 종료
  - 3자리보다 크거나 작은 값을 입력할 경우
  - 중복된 숫자를 입력할 경우
  - 1~9 사이의 숫자를 입력하지 않을 경우
  - 게임 종료 후, 1 or 2가 아닌 다른 수를 입력할 경우